...
async def status(ctx, name=None):
    ...
    if uuid == "0ce4275d3a1846eb909db576b74f9a55":
        ...
        embed.add_field(name='Ban timer', value=f"{unbanned} / {unbanned + 180} days", inline=False)
...
async def bw(ctx, name=None):
    ...
    embed.set_author(name='Jagares Bot', icon_url=f"https://minotar.net/helm/{ign}/400")
    embed.add_field(name='\u200B', value=f"[{ign}](https://plancke.io/hypixel/player/stats/{uuid})", inline=False)
    embed.add_field(name='Kills', value=f"`{'{:,}'.format(kills)}`", inline=False)
    embed.add_field(name='Deaths', value=f"`{'{:,}'.format(deaths)}`", inline=False)
    embed.add_field(name='K/D Ratio', value=f'`{kd}`', inline=False)
    embed.add_field(name='Final Kills', value=f"`{'{:,}'.format(final_kills)}`", inline=False)
    embed.add_field(name='Final Deaths', value=f"`{'{:,}'.format(fds)}`", inline=False)
    embed.add_field(name='FK/D Ratio', value=f'`{fkd}`', inline=False)
    embed.add_field(name=f'Finals For {next_fkd} FK/D Ratio', value=f"``{'{:,}'.format(need_fks)}``", inline=False)
    embed.add_field(name='Wins', value=f"`{'{:,}'.format(wins)}`", inline=False)
    embed.add_field(name='Losses', value=f"`{'{:,}'.format(losses)}`", inline=False)
    embed.add_field(name='W/L Ratio', value=f"`{'{:,}'.format(wl)}`", inline=False)
