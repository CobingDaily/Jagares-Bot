...
def get_final_kills_uuid(uuid, data_uuid):
    try:
        final_kills = int(data_uuid["player"]["stats"]["Walls3"]["final_kills"])
    except:
        final_kills = 0
    return final_kills
...
def get_final_assists_uuid(uuid, data_uuid):
    try:
        final_assists = int(data_uuid["player"]["stats"]["Walls3"]["final_assists"])
    except:
        final_assists = 0
    return final_assists


def get_final_deaths_post_uuid(uuid, data_uuid):
    try:
        final_deaths = int(data_uuid["player"]["stats"]["Walls3"]["final_deaths"])
    except:
        final_deaths = 0
    return final_deaths


def get_final_deaths_pre_uuid(uuid, data_uuid):
    try:
        finalDeaths = int(data_uuid["player"]["stats"]["Walls3"]["finalDeaths"])
    except:
        finalDeaths = 0
    return finalDeaths


def get_wins_uuid(uuid, data_uuid):
    try:
        wins = int(data_uuid["player"]["stats"]["Walls3"]["wins"])
    except:
        wins = 0
    return wins


def get_losses_uuid(uuid, data_uuid):
    try:
        losses = int(data_uuid["player"]["stats"]["Walls3"]["losses"])
    except:
        losses = 1
    return losses


def get_coins_uuid(uuid, data_uuid):
    try:
        coins = int(data_uuid["player"]["stats"]["Walls3"]["coins"])
    except:
        coins = 0
    return coins


def get_mythic_favor_uuid(uuid, data_uuid):
    try:
        mythic_favor = int(data_uuid["player"]["stats"]["Walls3"]["mythic_favor"])
    except:
        mythic_favor = 0
    return mythic_favor


def get_wither_damage_uuid(uuid, data_uuid):
    try:
        wither_damage = int(data_uuid["player"]["stats"]["Walls3"]["wither_damage"])
    except:
        wither_damage = 0
    return wither_damage


def get_chosen_class_uuid(uuid, data_uuid):
    try:
        chosen_class = str(data_uuid["player"]["stats"]["Walls3"]["chosen_class"])
    except:
        chosen_class = "none"
    return chosen_class

def get_chosen_skin_uuid(uuid, chosen_class, data_uuid):
    try:
        chosen_skin = str(data_uuid["player"]["stats"]["Walls3"][f"chosen_skin_{chosen_class.capitalize()}"])
    except:
        chosen_skin = "none"
    return chosen_skin


def get_cakes_found_uuid(uuid, data_uuid):
    try:
        cakes_found = int(data_uuid["player"]["stats"]["Walls3"]["cakes_found"])
    except:
        cakes_found = 0
    return cakes_found
